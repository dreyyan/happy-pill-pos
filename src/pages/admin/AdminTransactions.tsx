...
const AdminTransactions = () => {
  ...
  // * [UPDATE PAGE TITLE]
  usePageTitle(`Transactions: Admin | ${businessName}`);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${apiBase}/api/transactions/${deleteTarget.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token()}` },
      });
      if (res.status === 401) {
      ...
      if (viewingTxn?.id === deleteTarget.id) setViewingTxn(null);
  ...
  return (
    <div className="py-10 px-4 space-y-4 relative">
      ...
      {showDeleteModal && deleteTarget && (
        ...
        >
          <p>
            <strong>{deleteTarget.receiptNumber}</strong> will be removed from the active list.
            Stock will <strong>not</strong> be restored — void first if needed.
          </p>
        </Modal>
      )}
        ...
        <div className="fixed inset-0 z-40 flex">
          ...
          <div className="w-full max-w-lg bg-white shadow-2xl border-l border-[var(--color-bg-300)] flex flex-col overflow-y-auto">
            ...
            <div className="px-6 py-4 border-t border-[var(--color-bg-300)] flex gap-3">
              ...
              <button
                onClick={() => { setDeleteTarget(viewingTxn); setViewingTxn(null); setShowDeleteModal(true); }}
                className="flex-1 py-2 rounded-lg bg-[var(--color-red-500)] text-white font-roboto font-medium hover:bg-[var(--color-red-600)] transition-colors text-sm"
              >
                Remove
              </button>
            </div>
          ...
          <table className="min-w-full bg-white shadow-md table-auto border-collapse">
            <thead className="bg-[var(--color-primary-600)] text-white font-figtree">
              <tr>
                ...
                <th className="py-2 px-4 text-left font-bold border-r border-[var(--color-primary-600)] w-40 hidden sm:table-cell">Cashier</th>
            ...
            <tbody className="font-roboto">
              {displayedTransactions.map((txn) => (
                ...
                >
                  ...
                  <td className="text-sm py-2 px-4 text-[var(--color-text-700)] border-r border-[var(--color-bg-300)] hidden sm:table-cell">
                    {txn.cashier?.user?.firstName} {txn.cashier?.user?.lastName}
                  </td>
                  ...
                  <td className="py-2 px-4 flex justify-center items-center gap-2">
                    ...
                    <img
                      ...
                      alt="Remove"
                      className="w-5 h-5 cursor-pointer"
                      onClick={() => { setDeleteTarget(txn); setShowDeleteModal(true); }}
                    />
                  </td>
                </tr>
              ))}
