...
const AdminSalesReport = () => {
  usePageTitle("Sales Report: Admin | Happy-Pill Cafe");
  ...
  // * [UPDATE PAGE TITLE]
  usePageTitle(`Sales Report: Admin | ${businessName}`);
  ...
  const fetchReports = useCallback(async () => {
    try {
      ...
      // Use sensible defaults if user didn't pick dates
      const from = dateFrom || "2025-01-01";           // change to your earliest possible date
      ...
      // Backend already returns the exact shape your frontend expects
    ...
    <div className="py-10 px-4 space-y-6">
      ...
      <div className="flex flex-col items-center">
        ...
        <p className="text-sm font-roboto text-[var(--color-text-500)] mt-1">
          Comprehensive overview of sales, profit, and transaction trends
        </p>
      </div>
      ...
      {!loading && !error && reports.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 space-y-2 text-center">
          ...
          <p className="font-roboto text-sm text-[var(--color-text-500)]">
            Try adjusting the date range or generate reports from transactions.
          </p>
        </div>
      )}
          ...
          <div className="bg-white border border-[var(--color-bg-200)] rounded-lg p-4 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <Legend
                  wrapperStyle={{ fontSize: 12, fontFamily: "Roboto, sans-serif" }}
                />
                <Line
                  type="monotone"
                  dataKey="Sales"
