...
// * [PUT] Change Current Cashier Password
// ? /api/cashier/change-password
router.put('/', verifyRole(['CASHIER']), async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        info(`Cashier password changed successfully for user id ${currentUserId}`);
        ...
        error(`Error changing cashier password: ${errorMessage}`);
